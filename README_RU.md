...
🚴 Это Велокомпьютер на базе микроконтроллера ESP32, который отображает скорость и пройденное расстояние, помогает велосипедисту и даже подсказывает, когда нужно сбросить скорость!
...
### 🦵 Нагрузка и здоровье
- **Нагрузка на колени** — моя собственная разработка. Она анализирует скорость, пройденное расстояние и уклон дороги.
  | ⚠️ ВНИМАНИЕ! |
  |--------|
  **Расчетная нагрузка — это экспериментальный показатель интенсивности поездки, вычисляемый на основе скорости, расстояния и уклона. Он не является медицинским показателем.**
  
поездки, рассчитанный по скорости, расстоянию и уклону.
Не является медицинским измерением.**
...
**❗Для более точных показаний подтяните резистор на 10 кОм между DATA и VCC.**
...
- ЕСЛИ У ВАС УСТАНОВЛЕН более точный модуль DHT22, за место DHT11, то вам нужно заменить #define DHTTYPE DHT11 на #define DHTTYPE DHT22 в начале кода.
- Подключите ESP32 к компьютеру
- Выберите **Инструменты → Плата → ESP32 Dev Module**
- Выберите правильный **порт**
- Нажмите **Загрузить**
- Зажмите на плате кнопку "boot" для перехода платы в режим прошивки (Если плата не переходит в режим прошивки, то попробуйте перевести в режим прошивки принудительно, соединив пин GPIO0 на плате с пином GND с помощью обычной перемычки (куска провода))
