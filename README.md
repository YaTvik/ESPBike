...
**Версия 9.2 | Open Source (MIT) | На базе ESP32**
...
**ESPBike** — это мой первый серьёзный проект на ESP32. Я собрал велокомпьютер, который показывает скорость, дистанцию, помогает следить за нагрузкой на колени, подсказывает оптимальный темп и даже определяет уклон дороги.

Проект полностью рабочий, проверен на практике. Весь код и схема подключения — в открытом доступе.

**Важно:** при создании прошивки я использовал помощь искусственного интеллекта (ChatGPT). Это мой первый проект такого уровня, и ИИ помог мне разобраться с тонкостями программирования ESP32, работой с датчиками и отладкой. Я считаю, что это нормальная практика для начинающих — учиться с помощью современных инструментов. Весь код протестирован и работает на реальном устройстве.
...
| От внешнего аккумулятора | Зависит от ёмкости |

**Режим сна:** если 10 минут нет касаний экрана, устройство переходит в спящий режим. Коснитесь экрана — проснётся.
...
|--------|-------------|----------------|
| **ESP32** | Главный процессор | ~600 ₽ |
| **TFT экран 2.8"** | Показывает данные | ~800 ₽ |
| **GPS модуль NEO-6M** | Измеряет скорость | ~500 ₽ |
...
**Ответ:** Проверьте питание. Если используете 18650 — возможно, он разрядился. Подключите USB, подождите минуту, попробуйте снова.
...
- **OpenAI ChatGPT** — помощь в написании и отладке кода
- **Библиотеки:** TFT_eSPI, SparkFun u-blox GNSS, TinyGPS++
- **Сообщество ESP32** — за примеры и советы
